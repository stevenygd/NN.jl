type Sigmoid <: Nonlinearity
    ...
    function Sigmoid()
        return new(Layer[], Layer[], false, Base.Random.uuid4(), Float64[], Float64[], Float64[], Float64[])
    end

    function Sigmoid(prev::Union{Layer,Void}; config::Union{Dict{String,Any},Void}=nothing, kwargs...)
        layer = new(Layer[], Layer[], false, Base.Random.uuid4(), Float64[], Float64[], Float64[], Float64[])
