...
function beforeTest(smaxcross, smax, cross)
    init(smaxcross, nothing, Dict{String, Any}("batch_size" => 2, "input_size" => [3]))
    init(smax     , nothing, Dict{String, Any}("batch_size" => 2, "input_size" => [3]))
    init(cross    , nothing, Dict{String, Any}("batch_size" => 2, "input_size" => [3]))
end

function testSoftMaxCrossEntropyOneVector(smaxcross, smax, cross, x, label; alpha = 1.)
    beforeTest(smaxcross, smax, cross)
